...
class Person implements IAT{
    ...
    public boolean samePerson(Person that ) {
        return this.name.equals(that.name)&& this.yob == that.yob && this.isMAle == that.isMAle && this.mom.sameIAT(that.mom) && this.dad.sameIAT(that.dad);
    }
...
class Examples {
    ...
    IAT claire2 = new Person("Claire", 1956, false, this.darcy, new Unknown());
    IAT enid2 = new Person("Enid", 1904, false, new Unknown(), new Unknown());
        boolean testSameIAT(Tester t) {
        // Comparing squares
        t.checkExpect(andrew.sameIAT(bill), false);
        t.checkExpect(new Unknown().sameIAT(new Unknown()), true);
            t.checkExpect(andrew.sameIAT(andrew), true);
            t.checkExpect(claire.sameIAT(claire2), true);
            t.checkExpect(claire2.sameIAT(claire), true);
            t.checkExpect(enid.sameIAT(enid2), true);
